...
const ClientZoneAddressCell: FC<Props> = ({ client }) => {
    const inactive = client.status === 0;

    return (
        <div className="flex flex-col gap-0.5 min-w-[140px]">
            <span className={cn("font-semibold text-sm line-clamp-1", inactive && "text-destructive")}>
                {client.zone?.name || "—"}
            </span>
...
export default ClientZoneAddressCell;
