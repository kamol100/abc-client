...
export const SubZoneFormSchema = z.object({
    name: z.string({
        required_error: "sub_zone.name.errors.required",
        invalid_type_error: "sub_zone.name.errors.required",
    }).min(2, { message: "sub_zone.name.errors.min" }),
    name_bn: z.string().nullable().optional().default(""),
    zone_id: z.coerce.number({
        required_error: "sub_zone.zone.errors.required",
        invalid_type_error: "sub_zone.zone.errors.required",
    }).min(1, { message: "sub_zone.zone.errors.required" }),
    network_id: z.coerce.number().nullable().optional(),
